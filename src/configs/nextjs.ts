...
export async function nextjs(
  ...
  function getRules(name: keyof typeof pluginNextJS.configs): Record<string, any> {
    ...
    if (!rules)
      throw new Error(`[@zhangwj0520/eslint-config] Failed to find config ${name} in @next/eslint-plugin-next`);
    return normalizeRules(rules);
  }

  return [
    {
      name: 'zhangwj0520/nextjs/setup',
      plugins: {
        next: pluginNextJS,
      },
    },
